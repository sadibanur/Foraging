...
public class View {
    ...
    public Forager makeForager() {
        ...
        result.setState(io.readRequiredString("State: "));
    ...
    public void displayKgItem(Item key, BigDecimal value) {
        value = value.setScale(2, RoundingMode.HALF_UP);
        io.println(key.getName() + ": " + value + " kg");
    }

    public void displayCategoryValue(Map<Category, BigDecimal> values) {
        for (Map.Entry<Category, BigDecimal>entry : values.entrySet()) {
            Category category = entry.getKey();
            BigDecimal value = entry.getValue();

            value = value.setScale(2, RoundingMode.HALF_UP);
            io.println(category + ": $" + value );
        }
    }
}
